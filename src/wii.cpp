...
std::list<discovered_dev_t> Wii::DeviceList() {
  std::unique_lock<std::mutex> lock{m_};

  device_id = hci_get_route(NULL);
  socket_id = hci_open_dev(device_id);

  int max = 255;

  if (device_id < 0 || socket_id < 0) {
    std::cout << " get the device ID  failed exit " << endl;  //
                                                              // throw std::runtime_error("Failed creating HCI controller");
  }

  // if (device_id < 0 || socket_id < 0 || hci_devinfo(device_id, &info_) < 0) {
  //	throw std::runtime_error("Failed creating HCI controller");
  // }

  std::list<discovered_dev_t> devices;
  ...
  inq_inf = (inquiry_info*)malloc(max * sizeof(inquiry_info));
  // inquiry_info* ii = new inquiry_info[max * sizeof(inquiry_info)];
  // inquiry_info* inquiries = static_cast<inquiry_info*>(::operator new[](max *
  // sizeof(inquiry_info)));

  int resps = hci_inquiry(device_id, 5, max, NULL, &inq_inf, IREQ_CACHE_FLUSH);
  if (resps < 0) {
  ...
  for (int i = 0; i < resps; ++i) {
    std::cout << " d " << i << endl;  //
    char baddr_str[20] = {0};
    ba2str(&inq_inf[i].bdaddr, baddr_str);
    discovered_dev_t nearby_dev{name(inq_inf[i].bdaddr),
                                std::string{baddr_str}};

    devices.emplace_back(nearby_dev);
  }
...
std::string Wii::name(bdaddr_t const& dev) const {
  char cstr[64];
  std::string str{"unknown"};

  if (hci_read_remote_name(socket_id, &(dev), sizeof(cstr), cstr, 0) >= 0) {
    str = cstr;
  }
...
int Wii::Send(uint8_t report_type, uint8_t* msg, int len) {
  ...
  switch (report_type) {
    ...
    case WM_CMD_REPORT_TYPE: {
      ...
      if (WIIMOTE_IS_STATE_SET(state, WIIMOTE_STATE_RUMBLE)) rumble = 1;
      break;
    }
    default:
      break;
...
int Wii::Set_Report_Type() {
  ...
  buf[0] = (WIIMOTE_IS_FLAG_SET(flags, WIIC_CONTINUOUS)
                ? 0x04
...
int Wii::Pool() {
  ...
  if (r == -1) {
    ...
    if (errno == ENOTCONN) {
      ...
      WIIC_ERROR(
          "Bluetooth appears to be disconnected.  Wiimote unid will be "
          "disconnected.");
      Disconnect();
...
void Wii::event_status(uint8_t* msg) {
  ...
  WM_CTRL_STATUS_BYTE1_SPEAKER_ENABLED) WIIMOTE_ENABLE_STATE(wm,
...
Wii::EventTypes Wii::GetEvent() { return event; }

bool Wii::Connected() {
  if (WIIMOTE_IS_CONNECTED(state)) return true;
  return false;
}
...
float Wii::GetBatteryLevel() { return battery_level; }

int Wii::GetHandshakeState() { return handshake_state; }

bool Wii::RumbleEnabled() {
  if (WIIMOTE_IS_STATE_SET(state, WIIMOTE_STATE_RUMBLE)) return true;
  return false;
}
...
void Wii::Rumble(int status) {
  ...
  /* preserve IR state */
  if (WIIMOTE_IS_STATE_SET(state, WIIMOTE_STATE_IR)) buf |= 0x04;
