...
#define WIIMOTE_STATE_HANDSHAKE \
  0x00002 /* actual connection exists but no handshake yet */
#define WIIMOTE_STATE_HANDSHAKE_COMPLETE \
  0x00004 /* actual connection exists but no handshake yet */
...
#define WIIMOTE_STATE_EXP_HANDSHAKE                                          \
  0x04000 /* Added to handle handshaking concurrency between Motion Plus and \
             other expansion types */
#define WIIMOTE_STATE_EXP_FAILED 0x08000
#define WIIMOTE_STATE_MOTION_PLUS \
  0x10000                                  /**< MotionPlus state flag, set when the MotionPlus is used */
...
#define WIIMOTE_IS_STATE_SET(state, s) ((state & (s)) == (s))
#define WIIMOTE_ENABLE_STATE(state, s) (state |= (s))
#define WIIMOTE_DISABLE_STATE(state, s) (state &= ~(s))
#define WIIMOTE_TOGGLE_STATE(state, s)             \
  ((state & (s)) ? WIIMOTE_DISABLE_STATE(state, s) \
                 : WIIMOTE_ENABLE_STATE(state, s))

/* macro to manage flags */
#define WIIMOTE_IS_FLAG_SET(flags, s) ((flags & (s)) == (s))
#define WIIMOTE_ENABLE_FLAG(flags, s) (flags |= (s))
#define WIIMOTE_DISABLE_FLAG(flags, s) (flags &= ~(s))
#define WIIMOTE_TOGGLE_FLAG(flags, s)             \
  ((flags & (s)) ? WIIMOTE_DISABLE_FLAG(flags, s) \
                 : WIIMOTE_ENABLE_FLAG(flags, s))

#define NUNCHUK_IS_FLAG_SET(flags, s) ((*(flags) & (s)) == (s))

/* misc macros */
#define WIIMOTE_IS_CONNECTED(state) \
  (WIIMOTE_IS_STATE_SET(state, WIIMOTE_STATE_CONNECTED))
...
#define WIIC_USING_ACC(wm) \
  ((wm->state & WIIMOTE_STATE_ACC) == WIIMOTE_STATE_ACC)
#define WIIC_USING_EXP(wm) \
  ((wm->state & WIIMOTE_STATE_EXP) == WIIMOTE_STATE_EXP)
#define WIIC_USING_IR(wm) ((wm->state & WIIMOTE_STATE_IR) == WIIMOTE_STATE_IR)
#define WIIC_USING_SPEAKER(wm) \
  ((wm->state & WIIMOTE_STATE_SPEAKER) == WIIMOTE_STATE_SPEAKER)
#define WIIC_SPEAKER_MUTED(wm) \
  ((wm->state & WIIMOTE_STATE_SPEAKER_MUTE) == WIIMOTE_STATE_SPEAKER_MUTE)
#define WIIC_USING_MOTION_PLUS(wm) \
  ((wm->state & WIIMOTE_STATE_MOTION_PLUS) == WIIMOTE_STATE_MOTION_PLUS)
#define WIIC_IS_LED_SET(wm, num) \
 ...
 */
struct wiimote_t;
typedef void (*wiic_read_cb)(struct wiimote_t* wm, uint8_t* data,
                             unsigned short len);
 ...
 */
typedef enum aspect_t { WIIC_ASPECT_4_3,
                        WIIC_ASPECT_16_9 } aspect_t;
